...
make_param <- function(var) {
  val <- system(paste0("make print-", var), intern=TRUE, ignore.stderr=TRUE)
  strsplit(val, "=", fixed=TRUE)[[1]][2]
  }
...
load_params <- function(prefix, extra=FALSE) {
    ...
    if (extra) {
      if (file.exists(fname <- paste0(prefix, ".yml"))) {
        yml_params <- yaml::read_yaml(fname)[[1]]$params
      } else {
        yml_params <- list()
      }
      if (file.exists(fname <- file.path(dirname(prefix), "params.yml"))) {
        global_params <- yaml::read_yaml(fname)
      } else {
       global_params <- list()
      }
